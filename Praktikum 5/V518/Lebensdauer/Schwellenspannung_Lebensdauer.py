...
plt.figure()
plt.errorbar(data[0], d2y_dx2, xerr=xerr, yerr=d2y_dx2_err, label="Messwerte", color="black", linewidth=0.3, zorder=3, alpha=1, fmt='o', ms=3)

plt.xlabel(r'Spannung $-U$ / mV')
plt.ylabel(r'Ableitung $d(N_\mathrm{mess}/N_\mathrm{monitor})/dU$')
plt.axvline(x=1600, color='red', linestyle='-', linewidth=1)
plt.legend()
plt.grid(True)
