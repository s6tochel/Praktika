...
E_lit_Eu = np.array([121.7817, 244.6974, 344.2785, 778.9045, 867.380, 1100.9206, 1408.013])
E_lit_Eu_err = np.array([0.0003, 0.0008, 0.0012, 0.0024, 0.005, 0.006, 0.003])
...
E_lit_no_fit = E_lit_Eu[takeout_show[0]:takeout_show[-1]+1]
b = np.delete(b, takeout_show)
E_lit_Eu = np.delete(E_lit_Eu, takeout_show)
b_err_no_fit = b_err[takeout_show[0]:takeout_show[-1]+1]
E_lit_err_no_fit = E_lit_Eu_err[takeout_show[0]:takeout_show[-1]+1]
b_err = np.delete(b_err, takeout_show)
E_lit_Eu_err = np.delete(E_lit_Eu_err, takeout_show)
...
plt.errorbar(E_lit_no_fit, b_no_fit, yerr=b_err_no_fit, xerr=E_lit_err_no_fit, fmt='o', label=f'Nicht gefitteter Eu Messwert', color='g', ms=4, zorder=3, alpha=0.5)
...
plt.plot(fit_vals, linear(fit_vals, *popt), label=r"Linearer Fit ($\chi^2 \approx $" + f"{int(np.round(chi_squared,0))}" + ")", color="black", linewidth=1, zorder=2, alpha=0.8)

plt.xlabel("Literaturwerte $E_\gamma$ der Peak-Energien")
...
to_latex_table([b, b_err, E_lit, E_lit_err], dir_path + "/nai_tabelle.txt", round_to=[3, 3, 4, 4])